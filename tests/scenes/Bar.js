...
export default class Bar extends Phaser.Scene
{
    ...
    create()
    {
		...
		let bar = this.add.text(screenWidth/2 - 200, screenHeight/2 - 100, 'Prospects Bar', {fontFamily: 'CustomFont', fontSize: '40px', color: '#f65900', backgroundColor: '#ffff', padding: {x: 10, y: 10}});
		bar.depth = 1

		for(let i = 0, j = 22; i < 26; i++){
			platform.create(0 + j, screenHeight + 6, 'piso').setScale(3, 3).refreshBody()
			j += 48;
		}
		walls.depth = -1;
		...
		for(let i = 0, j = 20; i < 5; i++, j+=26){
			...
			if(choose == 1){
				let garrafa = bootles.create(screenWidth - 230 + j, screenHeight - 240, 'garrafa_1')
				garrafa.depth = 1
				garrafa.setScale(3, 3)
			}
			else if(choose == 2){
				let garrafa = bootles.create(screenWidth - 230 + j, screenHeight - 240, 'garrafa_2')
				garrafa.depth = 1
				garrafa.setScale(3, 3)
			}
			else {
				let garrafa = bootles.create(screenWidth - 230 + j, screenHeight - 240, 'garrafa_3')
				garrafa.depth = 1
				garrafa.setScale(3, 3)
			}
		}
		...
		for(let i = 0, j = 20; i < 5; i++, j+=26){
			let choose = parseInt(Math.random() * 3)
			if(choose == 1){
				let garrafa = bootles.create(screenWidth - 230 + j, screenHeight - 180, 'garrafa_1')
				garrafa.depth = 1
				garrafa.setScale(3, 3)
			}
			else if(choose == 2){
				let garrafa = bootles.create(screenWidth - 230 + j, screenHeight - 180, 'garrafa_2')
				garrafa.depth = 1
				garrafa.setScale(3, 3)
			}
			else {
				let garrafa = bootles.create(screenWidth - 230 + j, screenHeight - 180, 'garrafa_3')
				garrafa.depth = 1
				garrafa.setScale(3, 3)
			}
		}
		...
		this.chair = this.physics.add.existing(new Item(this, screenWidth/2, screenHeight - 78, 'cadeira')).setScale(4, 4)
		this.physics.add.collider(this.chair, platform)

		this.mesa = this.physics.add.image(new Item(this, screenWidth/2 + 70, screenHeight - 53, 'mesa').setScale(3, 3))
		this.physics.add.collider(this.mesa, platform)

		this.copo = this.physics.add.image(new Item(this, screenWidth/2 + 75, screenHeight - 75, 'copo_2').setScale(3, 3))
		this.physics.add.collider(this.copo, this.mesa)

		this.garrafa = this.physics.add.image(new Item(this, screenWidth/2 + 50, screenHeight - 80, 'garrafa_1').setScale(3, 3))
		this.physics.add.collider(this.garrafa, this.mesa)

		this.chair2 = this.physics.add.image(new Item(this, screenWidth/2 + 140, screenHeight - 80, 'cadeira', true).setScale(4, 4))
		this.physics.add.collider(this.chair2, platform)

		this.mesa2 = this.physics.add.image(new Item(this, screenWidth/2 - 250, screenHeight - 53, 'mesa').setScale(3, 3))
		this.physics.add.collider(this.mesa2, platform)

		this.copo2 = this.physics.add.image(new Item(this, screenWidth/2 - 260, screenHeight - 80, 'copo_2').setScale(3, 3))
		this.physics.add.collider(this.copo2, this.mesa)

		this.garrafa2 = this.physics.add.image(new Item(this, screenWidth/2 - 235, screenHeight - 80, 'garrafa_3').setScale(3, 3))
		this.physics.add.collider(this.garrafa2, this.mesa)

		this.chair3 = this.physics.add.existing(new Item(this, screenWidth/2 - 320, screenHeight - 130, 'cadeira')).setScale(4, 4)
		this.physics.add.collider(this.chair3, platform)

		this.chair4 = this.physics.add.image(new Item(this, screenWidth/2 - 180, screenHeight - 80, 'cadeira', true).setScale(4, 4))
		this.physics.add.collider(this.chair4, platform)

		this.lata = this.physics.add.image(new Item(this, 160, screenHeight - 68, 'lata_lixo', true).setScale(3, 3))
		this.physics.add.collider(this.lata, platform)
		
		this.prateleira = this.physics.add.image(new Item(this, screenWidth - 150, screenHeight - 200, 'prateleira', true).setScale(3, 3))
		this.prateleira2 = this.physics.add.image(new Item(this, screenWidth - 150, screenHeight - 140, 'prateleira', true).setScale(3, 3))
		
		this.balcao = this.physics.add.image(new Item(this, screenWidth - 150, screenHeight, 'balcao', true).setScale(3, 3))
		this.balcao.depth = 3
		this.physics.add.collider(this.balcao, platform)
		
		this.nex = this.physics.add.image(new Item(this, screenWidth/2, screenHeight - 50, 'nex', true).setScale(3, 3))
		this.nex.depth = 1
		this.physics.add.collider(this.nex, platform)

		this.sangue_parede = this.physics.add.image(new Item(this, screenWidth/2, screenHeight + 20, 'sangue_parede', true))
		this.sangue_parede.depth = -1
		this.sangue_parede2 = this.physics.add.image(new Item(this, screenWidth/2, screenHeight - 100, 'sangue_parede', true))
